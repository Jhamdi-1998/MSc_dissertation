...
"""
Loading the formation-period prices
"""
...
def adf_precondition_test(series: pd.Series) -> dict:
    ...
    return {
        ...
        "5%": crit_values["5%"],
        "10%": crit_values["10%"],
        "Fails to Reject H0": pvalue > 0.05,
    }
...
def main():
    ...
    n_cointegrated=summary_table["Rejects H0 (cointegrated)"].sum()
    print(f"\n{n_cointegrated}/{len(config.PAIRS)} pairs cointegrated at 5%.")
    if n_cointegrated < len(config.PAIRS):
        failed=summary_table[~summary_table["Rejects H0 (cointegrated)"]].index.tolist()
        print(f"NOT cointegrated (fail): {failed}")
